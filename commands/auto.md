...
If `blocked` → **PAUSE**:
- Say: "All remaining tasks are blocked. Blocked: {blocked_tasks}. In progress: {in_progress_tasks}."
- Log the state:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/progress.py append-structured --action task_fail --phase execute --track 2 --details '{"blocked_tasks": [...]}' "Auto-loop paused: all tasks blocked"
```
...
2. **Blocked tasks** → all remaining tasks have unmet dependencies
